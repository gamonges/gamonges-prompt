...
マージ済みの specs/ が `openspec validate --strict` を通過することを確認する:

```bash
pnpm exec openspec validate {domain} --strict
```

- 検証エラーが出た場合は次のフェーズに進まず、delta spec とのマージ結果を見直す
...
- [ ] `openspec/specs/{domain}/spec.md` に書き込む:
  ...
  - **追記**: 既存の `## Requirements` セクションに新しい Requirement を追加する。既存の内容は変更しない
...
- `pnpm exec openspec` が利用不可な環境（依存未インストール等）では警告のみ表示してスキップする
