...
2. **社会技術的距離**: 同一チームが保守しているか、別チームかという組織構造の影響。**注意**: リポジトリ/デプロイ単位の分離という物理的距離の事実だけから推測しない。対象コードの実際の担当チーム（組織的な保守境界）を特定した上で判定する。チーム編成はレイヤー別とは限らない（機能/ドメイン単位で一気通貫保有する編成もありうる）ため、物理的距離が高くても社会技術的距離は低い、という非対称な評価が起こりうる
...
実コードが存在するスキル（`coupling-audit` / `coupling-plan-diff`）と、実装前の意図ベース資料のみを対象とするスキル（`coupling-gate`: plan.md / `coupling-precheck`: context.md 等）とでは、結合関係テーブルの**列の中身**が異なる。表の列構成（列名）自体は4スキル共通。

| | 事実ベース（`coupling-audit` / `coupling-plan-diff`） | 推測ベース（`coupling-precheck` / `coupling-gate`） |
|---|---|---|
| 「結合関係」列 | フルクラス名必須 | 想定モジュール名/概念名 |
| 「根拠」列 | `file:L{number}` | `plan.md:L{number}`（`coupling-gate`）/ `context.md:L{number}`（`coupling-precheck`）または `(推測)` タグ |
